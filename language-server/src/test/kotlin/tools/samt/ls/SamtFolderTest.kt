...
class SamtFolderTest {
    ...
    fun `getFilesIn returns files in directory`() {
        val folder = SamtFolder("file:///tmp/test/samt.yaml".toPathUri(), "file:///tmp/test/src".toPathUri())
        val file1 = "file:///tmp/test/src/dir/foo.samt".toPathUri()
        val file2 = "file:///tmp/test/src/dir/bar.samt".toPathUri()
        val file3 = "file:///tmp/test/src/baz.samt".toPathUri()
        ...
        assertEquals(setOf(file1, file2), folder.getFilesIn("file:///tmp/test/src/dir".toPathUri()).toSet())
    }

    @Test
    fun `getMessages includes parser and semantic messages`() {
        val folder = SamtFolder("file:///tmp/test/samt.yaml".toPathUri(), "file:///tmp/test/src".toPathUri())
        val uri1 = "file:///tmp/test/src/record.samt".toPathUri()
        val uri2 = "file:///tmp/test/src/service.samt".toPathUri()
